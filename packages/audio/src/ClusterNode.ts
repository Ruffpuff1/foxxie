...
export class ClusterNode extends BaseNode {
	...
	public override emit(event: ConnectionEvents.Close, code: number, reason: string): boolean;
	public override emit(event: ConnectionEvents.Error, error: Error): boolean;
	public override emit(event: ConnectionEvents.Event, payload: IncomingEventPayload): boolean;
	public override emit(event: ConnectionEvents.Open): boolean;
	public override emit(event: ConnectionEvents.PlayerUpdate, payload: IncomingPlayerUpdatePayload): boolean;
	public override emit(event: ConnectionEvents.Stats, payload: IncomingStatsPayload): boolean;
	public override emit(event: ConnectionEvents.Upgrade, req: IncomingMessage): boolean;
	public override emit(event: ConnectionEvents, ...args: readonly any[]): boolean {
	...
	public override async destroy(code?: number, data?: string): Promise<void> {
