...
export class UserCommand extends Command {
    public async chatInputRun(...[interaction, , args]: ChatInputArgs<CommandName.Wolfram>): Promise<unknown> {
        ...
        const result = await centra('http://api.wolframalpha.com/v1').path('result').query('appid', process.env.WOLFRAM_TOKEN).query('i', query).text();
    ...
    private async graphical(interaction: Parameters<ChatInputCommand['chatInputRun']>['0'], query: string) {
        const { statusCode, body } = await centra('http://api.wolframalpha.com/v1')
            .path('simple')
