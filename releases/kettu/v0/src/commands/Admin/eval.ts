...
export class UserCommand extends Command {
    ...
    private async eval(_interaction: CommandInteraction, { message, code, async, depth, showHidden }: EvalParameters) {
        ...
        try {
            ...
            // @ts-expect-error value is never read, this is so `t` is possible as an alias when sending the eval.
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const t = getLocale(_interaction);

            // @ts-expect-error value is never read, this is so `interaction` is possible as an alias when sending the eval.
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    private async getHaste(result: string) {
        const { key } = await centra('https://paste.ruff.cafe') //
            .path('api') //
            .path('documents') //
            .body({ body: result }) //
