...
@ApplyOptions<FoxxieCommand.Options>({
    aliases: ['l'],
    description: languageKeys.commands.moderation.lockDescription,
    detailedDescription: languageKeys.commands.moderation.lockExtendedUsage,
...
export class UserCommand extends FoxxieCommand {
    ...
    public async messageRun(msg: GuildMessage, args: FoxxieCommand.Args): Promise<Message> {
        ...
        await send(msg, args.t(languageKeys.commands.moderation.lockLoading));
        await getModeration(msg.guild).actions.lock(
            {
        ...
        await sleep(700);
        return send(msg, args.t(languageKeys.commands.moderation.lockSuccess, { channel: channel.toString() }));
    }

    private checkLock(msg: GuildMessage, channel: TextChannel | NewsChannel | ThreadChannel): void {
        if (!channel.permissionsFor(msg.guild.id).has(PermissionFlagsBits.SendMessages)) this.error(languageKeys.commands.moderation.lockAlready, {
