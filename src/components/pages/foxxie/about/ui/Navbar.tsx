...
export default function Navbar({ hide }: { hide: boolean }) {
    ...
    return (
        <header
            className={`foxxie-navbar fixed z-40 flex w-full items-center justify-between bg-white px-3 py-2 ${scrolled ? 'border-b border-b-gray-200 shadow-md' : ''} ${
                hide ? 'top-[-75px]' : 'top-0'
            }`}
        >
            <button onClick={() => router.push('/foxxie/about')} className='group flex items-center space-x-3 rounded-md p-2 duration-300 hover:bg-gray-100'>
                <Image aria-hidden='true' src='https://cdn.reeseharlak.com/icons/foxxie.png' width={30} height={30} alt="Foxxie's logo" />
