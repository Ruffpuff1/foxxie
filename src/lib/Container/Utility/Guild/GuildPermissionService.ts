...
//         const { permissionsRoles, permissionsUsers } = await this.settings.readGuild(this.guild.id);

//         return {
//             everyone: permissionsRoles.find(r => r.id === this.guild.id) || { allowed: [], denied: [] },
//             roles: permissionsRoles.filter(r => r.id !== this.guild.id),
//             member: permissionsUsers
//         };
//     }
