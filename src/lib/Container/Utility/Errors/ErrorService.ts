...
import { UserCommand } from '#root/commands/Admin/eval';
...
export class ErrorService {
    ...
    private messageStringError(message: Message, t: TFunction, error: string) {
        return this.messageAlert(
            message,
            t(LanguageKeys.Listeners.Errors.String, { mention: message.author.toString(), message: error })
        );
    }
    ...
    private generateUnexpectedErrorMessage(
        userId: Snowflake,
        command: MessageCommand | Subcommand,
        t: TFunction,
        error: unknown
    ) {
