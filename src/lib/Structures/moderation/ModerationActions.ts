...
//             try {
//                 await cast<GuildTextBasedChannel>(this.guild.channels.cache.get(moderationLog.channelId!))?.bulkDelete(
//                     bulks
//                 );
//             } catch (error) {
//                 const handled = handleDiscordAPIError(error);
...
//         );
//         const { allow: prevAllow, deny: prevDeny } = rawChannel.permission_overwrites!.find(
//             perms => perms.id === this.guild.id
//         )!;
