import { TypeMetadata, TypeVariation } from '#lib/util/moderation';
import { createReferPromise, minutes, ReferredPromise, seconds } from '@ruffpuff/utilities';
import { AsyncQueue } from '@sapphire/async-queue';
import { container, UserError } from '@sapphire/framework';
import { isNullish } from '@sapphire/utilities';
import { Collection, Guild, GuildTextBasedChannelTypes, Snowflake } from 'discord.js';
...
export class GuildModerationService {
    public cache = new Collection<number, ModerationEntry>();
    ...
    public create<Type extends TypeVariation = TypeVariation>(
        data: GuildModerationService.CreateData<Type>
    ): GuildModerationService.Entry<Type> {
        return new ModerationEntry({
            id: 0,
            createdAt: 0,
            ...data,
            duration: data.duration ?? null,
            extraData: data.extraData ?? (null as GuildModerationService.ExtraData<Type>),
            guild: this.guild,
    ...
    public async insert(data: GuildModerationService.Entry): Promise<GuildModerationService.Entry> {
     ...
     */
    public async edit(entryOrId: GuildModerationService.EntryResolvable, data: GuildModerationService.UpdateData) {
     ...
     * @returns The updated moderation entry.
     */
    public async archive(entryOrId: GuildModerationService.EntryResolvable) {
        const entry = await this.#resolveEntry(entryOrId);
     ...
     * @returns The updated moderation entry.
     */
    public async complete(entryOrId: GuildModerationService.EntryResolvable) {
        const entry = await this.#resolveEntry(entryOrId);
     ...
     */
    public async delete(entryOrId: GuildModerationService.EntryResolvable) {
        const entry = await this.#resolveEntry(entryOrId);
     ...
     */
    public async fetch(id: number): Promise<GuildModerationService.Entry | null>;
    /**
     ...
     */
    public async fetch(options?: GuildModerationService.FetchOptions): Promise<Collection<number, GuildModerationService.Entry>>;
    public async fetch(
        options: number | GuildModerationService.FetchOptions = {}
    ): Promise<GuildModerationService.Entry | Collection<number, GuildModerationService.Entry> | null> {
        // Case number
    ...
    #addToCache(entries: ModerationEntry[], type: CacheActions): Collection<number, ModerationEntry>;
    #addToCache(
        entries: ModerationEntry | ModerationEntry[] | null,
        type: CacheActions
    ): Collection<number, ModerationEntry> | ModerationEntry | null {
    ...
    async #resolveEntry(entryOrId: GuildModerationService.EntryResolvable) {
    ...
    #getMany(options: GuildModerationService.FetchOptions): Collection<number, ModerationEntry> {
    ...
    async #fetchMany(options: GuildModerationService.FetchOptions): Promise<ModerationEntry[]> {
    ...
    async #performInsert(entry: GuildModerationService.Entry) {
    ...
    async #performUpdate(entry: GuildModerationService.Entry, data: GuildModerationService.UpdateData) {
...
export namespace GuildModerationService {
    export interface FetchOptions {
        userId?: Snowflake;
        moderatorId?: Snowflake;
    }

    export type Entry<Type extends TypeVariation = TypeVariation> = Readonly<ModerationEntry<Type>>;
