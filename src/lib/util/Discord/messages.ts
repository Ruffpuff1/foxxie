...
async function askConfirmationMessage(message: GuildMessage, response: GuildMessage, options: AskYesNoOptions) {
...
export async function messagePrompt(message: GuildMessage, options: AskYesNoOptions | string): Promise<null | boolean> {
    if (typeof options === 'string') options = { content: options };

    const response = await send(message, options);
    return canReact(response.channel)
        ? askYesOrNo(message, response, options)
        : askConfirmationMessage(message, cast<GuildMessage>(response), options);
}
