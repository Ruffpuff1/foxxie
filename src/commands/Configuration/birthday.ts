...
import { chunk, resolveToNull } from '@ruffpuff/utilities';
...
export class UserCommand extends FoxxieSubcommand {
    ...
    public async list(msg: GuildMessage, args: FoxxieCommand.Args): Promise<void> {
        ...
        const tasks = await fetchTasks(Schedules.Birthday, s => s.data.guildId === msg.guildId);
    ...
    public async set(msg: GuildMessage, args: FoxxieCommand.Args) {
        ...
        if (task) this.container.schedule.remove(task.id);
    ...
    private async mapBirthday(birthday: MappedTask<Schedules.Birthday>, t: TFunction, guild: Guild) {
        ...
        const age = getAge(
            { day: birthday.data.day, month: birthday.data.month, year: birthday.data.year },
            { now: birthday.time.getTime() }
        );
