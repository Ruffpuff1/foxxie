...
import { isGuildOwner } from '#lib/util/Discord/members';
import { sendLoadingMessage } from '#lib/util/Discord/messages';
import { floatPromise, resolveClientColor } from '#lib/util/util';
...
export class UserCommand extends FoxxieSubcommand {
    ...
    private async buildUserMessageDisplay(
        ...
        display.addAsyncPageEmbed(async embed => {
            const about: (string | null)[] = [
                args.t(LanguageKeys.Commands.General.InfoUserDiscordJoin, { created: user.createdAt })
            ];
