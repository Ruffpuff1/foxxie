...
module.exports = {
    ...
    english: {
        ...
        COMMAND_WELCOMEMESSAGE_DESCRIPTION: () => `Set the message I send when a new member joined the server. In your welcome message you can also use the variables \`{member}\`, \`{name}\`, \`{tag}\` (ex: ArEo#1245), \`{discrim}\`, \`{created}\`, \`{server}\`, and \`{count}\` for the server's member count.`,
        COMMAND_WELCOMEMESSAGE_NOMESSAGE: () => `There is no welcome message set right now, my default message is:\n\`\`\`**{member}** just joined the server!\`\`\`You can set your own message using the command \`fox welcomemessage (message)\``,
        COMMAND_WELCOMEMESSAGE_NOW: msg => `Right now the welcome message is set to:\n\`\`\`${msg}\`\`\`If ya wanna change it, use the command \`fox welcomemessage (message)\`.`,
        COMMAND_WELCOMEMESSAGE_REMOVED: () => `**Gotcha**, reset the welcome message back to the default of mine.`,
        COMMAND_WELCOMEMESSAGE_SET: msg => `**Gotcha,** set the welcome message to:\n\`\`\`${msg}\`\`\`Now I'll send the message with my welcome logging.`,
        ...
        COMMAND_CREATEKEY_NOID: () => `**Hey,** you either didn't provide an Id or the Id provided is invalid. The current valid Ids are \`0 - 4\`.`,
        ...
        COMMAND_RELOAD_DESCRIPTION: () => `Allows you to reload a specific \`command\` or \`monitor\` without having to restart the bot. This command is locked to the bot developer due to it's special nature.`,
        ...
        COMMAND_RELOAD_NOEXIST: (name) => `**Sorry,** \`${name}\` doesn't appear to be a valid command or monitor.`,
        ...
        COMMAND_STAB_FOOTER: (sender, reciever, authNum, recNum) => `${reciever} got stabbed ${recNum} times (ouch) and ${sender} slapped others ${authNum} times`,
        ...
        COMMAND_HELP_CATEGORY_AUTOMATION: () => `Set me to automatically do things around your server. Like welcome & goodbye messages, and Disboard reminders :)`,
        COMMAND_HELP_CATEGORY_EMBED_DESCRIPTION: (cate, language, lang) => `These are all of my **${cate.toLowerCase()}** commands.\n${language.get(`COMMAND_HELP_CATEGORY_${cate.toUpperCase()}`, lang)}\n\nAdditionally for each command you can user \`fox help (command)\` for a detailed description on that command, as well as example usage.`,
        COMMAND_HELP_CATEGORY_EMBED_TITLE: cate => `Foxxie's ${cate.toUpperCaseFirst()} commands!`,
        COMMAND_HELP_CATEGORY_FUN: () => `Random fun commands that will hopefully make people on your server laugh.`,
        COMMAND_HELP_CATEGORY_MODERATION: () => `Moderation commands to help you enforce rules and get the bad guys out.`,
        COMMAND_HELP_CATEGORY_ROLEPLAY: () => `Commands for expressing how you feel to people on your server. From warm hugs to murder!.`,
        COMMAND_HELP_CATEGORY_SETTINGS: () => `Configure how I work around your server. Like setting a custom prefix for me to respond to.`,
        COMMAND_HELP_CATEGORY_UTILITY: () => `Useful commands for getting you infomation or completing simple tasks for you.`,
        COMMAND_HELP_COMMAND_NOTVALID: () => `**Hey,** that's not a valid command!`,
        COMMAND_HELP_DESCRIPTION: () => `Will display information on my command categories or individual commands. You can also specify the \`-c\` or \`-channel\` flag to send the menu in the current channel.`,
        COMMAND_HELP_EMBED_AUTOMATION: size => `:robot: **Automation (${size})**`,
        COMMAND_HELP_EMBED_DESCRIPTION: (language, lang) => `These are all of my command categories. For a list of commands in a certain category do \`fox help (category)\`.\n\n${language.get('COMMAND_HELP_EMBED_DESCRIPTION_BIG', lang)}`,
        COMMAND_HELP_EMBED_DESCRIPTION_BIG: () => `Additionally for each command you can use \`fox help (command)\` for a detailed description on that command as well as example usage.\n\nFor arguments in commands:\n\`[Brackets]\` means the argument is __required__.\n\`(Parenthesis)\` means the argument is __optional__.\n\`(Two|options)\` means choose __either option__ for that argument.\n**Also don't include these symbols in your final commands-**`,
        ...
        LOG_MODERATION_EMBED_TITLE: act => `**${act} ${['Purged', 'Locked', 'Unlocked', 'Slowmoded'].includes(act) ? 'Channel' : 'User'}**`,
