...
module.exports = {
    ...
    name: 'en-US',
        ...
        COMMAND_RELOAD_ERROR: (name, err) => `**Error** reloading command **${name}**, \`\`\`js\n${err}\n\`\`\``,
        COMMAND_RELOAD_COMMAND_SUCCESS: (name, time) => `**Successfully** reloaded command **${name}**, (took ${time})`,
        ...
        COMMAND_RELOAD_ERROR: (name, err) => `**Error** reloading language **${name}**, \`\`\`js\n${err}\n\`\`\``,
        COMMAND_RELOAD_LANGUAGE_SUCCESS: (name, time) => `**Successfully** reloaded language **${name}**, (took ${time})`,
        COMMAND_RELOAD_ERROR: (name, err) => `**Error** reloading monitor **${name}**, \`\`\`js\n${err}\n\`\`\``,
        COMMAND_RELOAD_MONITOR_SUCCESS: (name, time) => `**Successfully** reloaded monitor **${name}**, (took ${time})`,
        ...
        COMMAND_KICK_DESCRIPTION: `Kick a member from the server. This will send a DM to the member telling them why they got kicked, along with log to a moderation channel if one is set.`,
        COMMAND_KICK_NOMEMBER: `**Please,** specify **one member** to kick.`,
        ...
        COMMAND_MUTE_DESCRIPTION: `Allows you to mute members of the server to prevent them from talking, this command will use a premade mute role by the bot and log to the moderation channel if set. You can also specify a time for the mute to automatically expire at such as \`1d\` for one day.`,
        ...
        COMMAND_PURGE_DESCRIPTION: `Clears out messages with optional filters, by default I won't clear important pinned messages.\n\`link\` will only clear links.\n\`invite\` will only clear discord invites.\n\`bots\` will only clear messages sent by bots.\n\`you\` will only clear messages sent by me.\n\`me\` will only clear messages sent by you.\n\`upload\` will only clear file uploads, like images and videos.\n\`text\` will only clear text, avoiding any images.\n\`user\` lets you specify a user to clear messages from, this can be their @mention or their user Id.`,
        COMMAND_PURGE_NOLIMIT: `**Please,** specify a proper number of messages to purge.`,
        COMMAND_PURGE_SUCCESS: num => `**Successfully** cleared out ${num} message${num === 1 ? '' : 's'} for you.`,
        COMMAND_PURGE_USER: `**Please,** specify **one user** to purge messages from.`,
        COMMAND_SLOWMODE_DESCRIPTION: `Set a slowmode in a channel so theres a wait between members sending messages. I can set a slowmode anytime between 21,600 seconds and 0 seconds. Alternatively you can specify \`none\` to remove any slowmode in a channel.`,
        COMMAND_SLOWMODE_INVALIDTIME: `**Sorry,** I can only set a slowmode between **21,600** and **0** seconds.`,
        COMMAND_SLOWMODE_NOARGS: `**Please,** specifiy the amount of seconds to set the slowmode for.`,
        COMMAND_SLOWMODE_NOTANUMBER: `**Whoops,** that doesn't look like a number.`,
        ...
        COMMAND_UNBAN_DESCRIPTION: `Unbans a user from the server. If possible will send a DM to the user notifying them that they have been unbanned. If a moderation logging channel is set, this action will log there.`,
        COMMAND_UNBAN_NOUSER: `**Please,** specify **one user** to unban.`,
        COMMAND_UNLOCK_DESCRIPTION: `Will unlock a channel if locked. This action will log to a moderation channel if one is set. Also note that this command only updates permissions for the @everyone permission, so any other roles with the \`SEND_MESSAGES\` permission will not be affected.`,
        COMMAND_UNLOCK_CHANNELNOTLOCKED: `**Whoops,** this channel isn't locked right now.`,
        COMMAND_UNLOCK_SUCCESS: `**Successfully** unlocked the channel for ya.`,
        COMMAND_UNLOCK_UNLOCKING: `${infinity} **Finding** the keys...`,
        ...
        COMMAND_UNMUTE_NOMEMBER: `**Please,** specify **one member** to unmute.`,
        COMMAND_UNMUTE_NOTMUTED: `**Whoops,** the member specified isn't currently muted.`,
        ...
        COMMAND_VCMUTE_DESCRIPTION: `Server mutes the specified member in a VC so they can no longer talk.`,
        COMMAND_VCMUTE_NOMEMBER: `You need to specify **one member** to vcmute.`,
        COMMAND_VCMUTE_NOVOICE: `**Hey,** looks like the member you're trying to vcmute isn't even in a VC.`,
        COMMAND_VCUNMUTE_DESCRIPTION: `Removes a server mute from the specified member in a VC so they can talk again. This action will log to a moderation channel if one is set.`,
        COMMAND_VCUNMUTE_NOMEMBER: `**Please,** specify **one member** to unmute in VC.`,
        COMMAND_VCUNMUTE_NOTMUTED: `**Hey,** the member you're trying to unmute currently isn't muted.`,
        COMMAND_VCUNMUTE_NOVOICE: `**Hey,** looks like the member you're trying to unmute isn't even in a VC.`,
        ...
        COMMAND_PREFIX_REMOVE_NOARGS: `**Please,** specify the prefix from this server you wanna remove.`,
        ...
        COMMAND_SETTINGS_DISBOARDS: (emoji, state, lang, language) => `${emoji} Disboard currently **${language.get(`COMMAND_SETTINGS_${state.toUpperCase()}`)}**`,
        COMMAND_SETTINGS_ENABLED: `enabled`,
        COMMAND_SETTINGS_GOODBYE: `goodbye`,
        COMMAND_SETTINGS_GOODBYES: (emoji, state, lang, language) => `${emoji} Goodbyes currently **${language.get(`COMMAND_SETTINGS_${state.toUpperCase()}`)}**`,
        ...
        COMMAND_SETTINGS_LOGGERS: (emoji, state, lang, language) => `${emoji} Logger currently **${language.get(`COMMAND_SETTINGS_${state.toUpperCase()}`)}**`,
        ...
        COMMAND_SETTINGS_STARBOARDS: (emoji, state, lang, language) => `${emoji} Starboard currently **${language.get(`COMMAND_SETTINGS_${state.toUpperCase()}`)}**`,
        ...
        COMMAND_SETTINGS_TITLE: (guild, setting, language, lang) => `__Foxxie's ${setting ? `${language.get(`COMMAND_SETTINGS_${setting.toUpperCase()}`)} ` : ''}settings in **${guild}**:__\n`,
        COMMAND_SETTINGS_USER: `user`,
        COMMAND_SETTINGS_WELCOME: `welcome`,
        COMMAND_SETTINGS_WELCOMES: (emoji, state, lang, language) => `${emoji} Welcomes currently **${language.get(`COMMAND_SETTINGS_${state.toUpperCase()}`)}**`,
        COMMAND_STARBOARD_CHANNEL_NOCHANNEL: `**Whoops,** there isn't a starboard channel set right now. Try setting one with \`fox starboard channel #channel\`.`,
        COMMAND_STARBOARD_CHANNEL_NOW: channel => `**Right now,** the starboard channel is set to <#${channel}>.`,
        COMMAND_STARBOARD_CHANNEL_REMOVED: `**Oki,** I removed the starboard channel for this server.`,
        COMMAND_STARBOARD_CHANNEL_SET: channel => `**Gotcha,** set the starboard channel to ${channel}.`,
        COMMAND_STARBOARD_DESCRIPTION: `Configure my starboard settings for your server using different arguments. Starboard is a democratic pinning system where I will send messages if they recieve a certain amount of :star: reactions.\n\nSpecify \`channel\` to set the channel where I send starboard messages.\n\nSpecify \`minimum\` to set the amount of reactions needed to get onto the starboard (default set to 3).\n\nSpecify \`self\` to configure whether reacting to your own message will count towards the starboard.\n\nSpecify \`notifs\` to configure whether I will send notifications when a message has been starred.`,
        COMMAND_STARBOARD_INVALIDUSE: `**Please,** specify a proper use case [channel|minimum|self|notifs].`,
        COMMAND_STARBOARD_MINIMUM_NOW: minimum => `**Right now,** the star minimum is set to **${minimum}**.`,
        COMMAND_STARBOARD_MINIMUM_REMOVED: `**Alrighty,** I'll reset back to my default minimum.`,
        COMMAND_STARBOARD_MINIMUM_SET: minimum => `**Done,** now you'll need **${minimum} star${minimum === 1 ? '' : 's'}** to get on the starboard.`,
        COMMAND_STARBOARD_NOTIFICATIONS_DENIED: `**Gotcha,** now I won't send notifications when a message has made it into the starboard.`,
        COMMAND_STARBOARD_NOTIFICATIONS_NOW: notif => `**Right now,** I ${notif === false ? 'won\'t' : 'will'} send notifications when a message gets on the starboard.`,
        COMMAND_STARBOARD_NOTIFICATIONS_REMOVED: `**Alright,** now I'll send notifications when a message has made it into the starboard.`,
        COMMAND_STARBOARD_SELF_DENIED: `**Gotcha,** now starring your own messages won't go towards your star count.`,
        COMMAND_STARBOARD_SELF_NOW: self => `**Right now,** starring your own messages ${self === false ? 'doesn\'t' : 'does'} count towards your star total.`,
        COMMAND_STARBOARD_SELF_REMOVED: `**Alright,** now starring your own messages will go towards your star count.`,
        ...
        COMMAND_INVITE_EMBED_TITLE: `Hey hey, here's some useful links.`,
        COMMAND_INVITE_EMBED_DESCRIPTION: [`[Invite me](${inviteURL})`, `[Support Server](${supportServer})`, `[Vote](${topggURL})`].join(' | '),
        ...
        COMMAND_MATH_INVALID: `**Sorry,** I can't calculate that expression, for examples of valid expressions try \`fox help math\`.`,
        COMMAND_MATH_NOARGS: `**Hey,** you need to enter something to calculate.`,
        COMMAND_PING: `Ping?`,
        ...
        COMMAND_POLL_DESCRIPTION: `Allows you to create polls for server members to vote on, separate poll options with a comma \`,\`. Additionally you can indicate the title of a poll using \`title=Title-Here\` using dashes \`-\` as spaces.`,
        COMMAND_POLL_EMBED_FOOTER: `React to the emojis below to vote in the poll.`,
        COMMAND_POLL_EMBED_TITLE: author => `Poll by ${author}`,
        COMMAND_POLL_OPTIONS: `**Sorry,** for polls you must have at least **two options** and no more than **ten options**.`,
