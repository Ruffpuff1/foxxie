...
@ApplyOptions<ListenerOptions>(({ container }) => ({ event: 'rawReactionRemove', enabled: container.client.enabledProdOnlyEvent() }))
export class UserListener extends Listener {
	public async run(channel: TextChannel, data: GatewayMessageReactionRemoveDispatch['d']) {
		...
		const emoji = getEmojiString({ name: data.emoji.name, id: data.emoji.id, animated: data.emoji.animated });
