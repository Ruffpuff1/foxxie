...
class GuildLogger {
    ...
    async send({ type, action, color, user, users, member, moderator, reason, duration, message, channel, oldMessage, newMessage, dm, msg, counter, total, warn, warns }) {
        ...
        const content = [
            ...
            type === 'mod'
                ...
                : null,
            warn && warnAuth
                ? '\n' + this.guild.language.get('LOG_ARGS_WARN', lang, warn.reason, warn.timestamp, warnAuth.tag, warnAuth.toString(), warnAuth.id)
                : null,
            warns
                ? '\n' + this.guild.language.get('LOG_ARGS_WARNS', lang, warns, this.guild.client)
                : null
    ...
    async dm({ user, users, action, moderator, reason, duration, guild, member }) {
        ...
        for (let user of users) {
            ...
            const embed = new MessageEmbed()
                ...
                .setTitle(guild.language.get(`LOG_DM_${action.toUpperCase()}`, lang))
                .setThumbnail(guild.iconURL({dynamic: true}))
                .setDescription(`${guild.language.get(`LOG_DM_${action.toUpperCase()}`, lang)} ${action.includes('ban') || action === 'kick'
                ? 'from' 
                : 'in'
            } **${guild.name}** ${duration 
                ? `for ${duration} `
