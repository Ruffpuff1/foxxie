...
class GuildLogger {
    ...
    constructor(guild) {
        this.guild = guild;
        this.timestamp = new Timestamp('llll')
    };
    ...
    async send({ type, action, color, user, users, member, moderator, reason, duration, message, channel, oldMessage, newMessage, dm, counter, total, warn, warns, oldMember, newMember }) {
        ...
        let date = this.timestamp.display(Date.now());
        ...
        if (user) user = this.guild.client.users.cache.get(user.id);
        ...
        const content = [
            ...
            type === 'mod' || type === 'member'
                ...
                : null,
            warn && warnAuth
                ? '\n' + this.guild.language.get('LOG_ARGS_WARN', warn.reason, this.timestamp.display(warn.timestamp), warnAuth.tag, warnAuth.toString(), warnAuth.id)
                : null,
            warns
                ? '\n' + this.guild.language.get('LOG_ARGS_WARNS', warns.map((w, idx) => `${idx + 1}. ${w.reason}\non ${this.timestamp.display(w.timestamp)}\nBy: ${this.guild.client.users.cache.get(w.author).tag} ${this.guild.client.users.cache.get(w.author).toString()} (ID: ${w.author})`).join('\n\n'))
                : null
    ...
    async dm({ user, users, action, moderator, reason, duration, guild, member }) {
        ...
        for (let user of users) {
            ...
            if (!user) continue;
            user = this.guild.client.users.cache.get(user.id);

            const embed = new MessageEmbed()
                .setColor(this.guild.me.displayColor)
                .setTitle(guild.language.get(`LOG_DM_${action.toUpperCase()}`))
                .setThumbnail(guild.iconURL({dynamic: true}))
                .setDescription(`${guild.language.get(`LOG_DM_${action.toUpperCase()}`)} ${action.includes('ban') || action === 'kick' || action === 'globalBan'
                ? 'from' 
                : 'in'
            } **${guild.name}** ${duration 
                ? `for ${duration} `
