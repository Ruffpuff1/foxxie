...
module.exports.mimuPick = (message) => {
    ...
    return message.responder.mimuPick(message)
}

module.exports.memberCount = (client) => {
    const updateMembers = () => {
        let guild = client.guilds.cache.get('761512748898844702')
        if (!guild) return;
...
module.exports.welcomeMessage = async (member) => {
   ...
   **•** If you have any questions feel free to reach out to staff or other server members. We hope you enjoy our shop!`);
   ...
   let welcomeChannel = await member.guild.settings.get('welcomeChannel')
   if (!welcomeChannel) return;

   welChn = member.guild.channels.cache.get(welcomeChannel)
