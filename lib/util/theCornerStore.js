...
module.exports.tcsWelcome = async (member) => {
   ...
   **•** If you have any questions feel free to reach out to staff or other server members. We hope you enjoy our shop!`);
   ...
   const msg = await welChn.send(`<@${member.id}> **Just joined the server. <@&774173127717552139> be sure to welcome them. We now have ${member?.guild?.memberCount} member${member?.guild?.memberCount > 1 ? 's' : ''}!**`, {embed: embed} )
