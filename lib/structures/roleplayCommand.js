...
class RoleplayCommand {
    ...
    async execute(act, args, self){
        ...
        const gifs = await this.message.client.framework.get(`gifs.roleplay`);
        ...
        if (this.message.mentions.members && this.message.mentions.members.size > 1) {
            description.push(language.get(`COMMAND_${act.toUpperCase()}_MULTIPLE`, this.message.member.displayName, this.message.mentions.members.array().slice(0, -1).map(m => m.displayName).join(", ") + `** and **${this.message.mentions.members.last().displayName}`));
            ...
            embed
                .setFooter(language.get(`COMMAND_${act.toUpperCase()}_FOOTER`, this.message.member.displayName, this.message.mentions.members.first().displayName, authorGiven.toLocaleString(), tarRecieved.toLocaleString()));
