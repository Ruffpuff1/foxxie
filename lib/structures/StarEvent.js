...
module.exports.StarEvent = async (reaction, user) => {
    ...
    if (reaction.message.channel.id === settings.channel || reaction.message.channel.id === "779239210360111116" || reaction.count < minimum || settings.nostar?.includes(reaction.message.channel.id) || settings.self === false && reaction.message.author.id === user.id || user.id === reaction.message.client.user.id || reaction.message.channel.id === settings.channel.id) return;
    ...
    function _notification () {
        ...
        const embed = new MessageEmbed()
            .setTitle(language.get('EVENT_STARBOARD_NOTIF_TITLE'))
            .setDescription(language.get('EVENT_STARBOARD_NOTIF_DESCRIPTION', reaction.message.author, channel, reaction.message.url))
