...
module.exports.StarEvent = async (reaction, user) => {
    ...
    if (reaction.count < minimum || settings.nostar?.includes(reaction.message.channel.id) || settings.self === false && reaction.message.author.id === user.id || user.id === reaction.message.client.user.id || reaction.message.channel.id === channel.id) return;
