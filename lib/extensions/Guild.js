...
Structures.extend('Guild', Guild => {
    class FoxxieGuild extends Guild {
        ...
        async createMuteRole(name) {
            ...
            const lang = await this.settings.get('language') || 'en-US';

            const muterole = await this.roles.create({
                data: {
                    name: name || this.language.get('COMMAND_MUTE_ROLE_DEFAULT', lang),
                    permissions: 0,
                    color: '#2f3136',
                    position: this.me.roles.highest.position
                },
                reason: this.language.get('COMMAND_MUTE_ROLE_REASON', lang)
            })
