...
Structures.extend('Message', Message => {
    class FoxxieMessage extends Message {
        ...
        getPrefix() {
            if (!this.guild) return this.prefixes = [this.client.mentionPrefix, this.client.development ? /^dev\s/i : /^fox\s/i, this.client.development ? new RegExp(`^${this.client.options.prefix.development}`) : new RegExp(`^${this.client.options.prefix.production}`)];
            return this.guild?.settings.get('prefixes').then(prefix => {

                let prefixes = [];
                if (!prefix?.length) return this.prefixes = [this.client.mentionPrefix, this.client.development ? /^dev\s/i : /^fox\s/i, this.client.development ? new RegExp(`^${this.client.options.prefix.development}`) : new RegExp(`^${this.client.options.prefix.production}`)];
                if (prefix.length) {
