...
Structures.extend('Message', Message => {
    class FoxxieMessage extends Message {
        constructor(client, data, channel){
            ...
            this.stringPrefixes = this.stringPrefixes || [`<@!${this.client.user.id}>`, this.client.development ? 'dev ' : 'fox ', this.client.development ? 'd.' : '.'];
        }
        ...
        getPrefix() {
            if (!this.guild) return [this.client.mentionPrefix, this.client.development ? /^dev\s/i : /^fox\s/i, this.client.development ? new RegExp(`^${this.client.options.prefix.development}`, 'i') : new RegExp(`^${this.client.options.prefix.production}`, 'i')];
            return this.guild?.settings.get('prefixes').then(prefix => {

                let prefixes = [];
                if (!prefix?.length) return [this.client.mentionPrefix, this.client.development ? /^dev\s/i : /^fox\s/i, this.client.development ? new RegExp(`^${this.client.options.prefix.development}`, 'i') : new RegExp(`^${this.client.options.prefix.production}`, 'i')];
                if (prefix.length) {
