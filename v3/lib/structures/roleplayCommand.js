...
class RoleplayCommand extends Command {
    ...
    async run(msg, args) {
        ...
        let target = msg.members.shift() || msg.guild.members.cache.get(args[0]) || msg.guild.members.cache.find(m => m.displayName.toLowerCase() === args[0]?.toLowerCase()) || msg.guild.members.cache.find(m => m.user.username.toLowerCase() === args[0]?.toLowerCase());
        const description = [];
    ...
    async _alone(msg, args, embed, description) {
        ...
        embed
            .setDescription(description.join('\n'))
            .setFooter(`${msg.member.displayName} has ${
                ['blush', 'facepalm', 'lurk'].includes(this.name)
                    ? `${this.name}ed`
                    : this.singleAction
                } ${authorGiven} times`
            )
