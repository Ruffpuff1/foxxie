...
class ModuleFile {
  ...
  public toExportDeclaration(
    ...
    const adjustedPath = (
      this.path.base === "index.ts"
        ? relativePath
        : join(relativePath, this.path.name)
    )
      .split(winSep)
      .join(posixSep);
...
async function findIndexOrModules(
  dir: string,
  depth: number = 0
): Promise<string[]> {
  ...
  for (const item of contents) {
    ...
    if (item.isFile()) {
      ...
      item.isDirectory() &&
      !contents.find((entry) => `${item.name}.ts` === entry.name)
    )
...
async function processPackage(packageDir: string): Promise<string> {
  ...
  const indexExternalExports = parseExternalExports(
  ...
  return PRINTER.printList(
    ...
    indexExternalExports.concat(
      modules
        .map((moduleFile) =>
          new ModuleFile(
            indexProgram.getSourceFile(moduleFile)!
          ).toExportDeclaration(packageDir)
        )
...
async function main() {
  ...
  const [currentFile, result] = [
    await readFile(join(packageDir, "index.ts"), { encoding: "utf-8" }),
