...
interface GraphQLConnection<
  T extends Github.User | Repository | Issue | PullRequest,
> {
...
export interface GhSearchIssuesAndPullRequestsParameters
  extends GhSearchRepositoriesParameters {
  owner: string;
  number: string;
}

export interface FetchIssuesAndPrsParameters
  extends GhSearchRepositoriesParameters {
  owner: string;
  number: number;
}
...
export async function fetchFuzzyRepo(owner: string, repository: string) {
  ...
  return result.value!.data.search!.nodes.filter(
    (repo) => cast<Github.Repo>(repo).visibility === "PUBLIC"
  );
}

export async function fetchIssuesAndPrs(
  { repository, owner, number }: FetchIssuesAndPrsParameters,
  t: TFunction
): Promise<IssueOrPrDetails> {
  ...
  if (
    isErr(result) ||
    (isNullish(result.value.repository?.pullRequest) &&
      isNullish(result.value.repository?.issue))
  ) {
...
export async function fuzzilySearchForIssuesAndPullRequests({
  ...
  owner,
  number,
}: GhSearchIssuesAndPullRequestsParameters): Promise<
...
> {
  ...
  if (
    isErr(result) ||
    (isNullishOrEmpty(result.value.repository?.pullRequests) &&
      isNullishOrEmpty(result.value.repository?.issues))
  ) {
    return [];
  }

  return getDataForIssuesAndPrSearch(
    number,
    result.value.repository?.pullRequests?.nodes,
    result.value.repository?.issues?.nodes
  );
}
...
function getDataForIssuesAndPrSearch(
  ...
  const issuesHasExactNumber = issues?.find(
    (issue) => issue.number === numberParsedNumber
  );

  if (issuesHasExactNumber) {
    const parsedIssueState =
      issuesHasExactNumber?.state === "OPEN" ? "Open Issue" : "Closed Issue";
  ...
  const pullRequestsHaveExactNumber = pullRequests?.find(
    (pullRequest) => pullRequest.number === numberParsedNumber
  );
  ...
  if (!isNullishOrEmpty(issues)) {
    for (const issue of issues) {
      ...
      if (!Number.isNaN(numberParsedNumber)) {
        if (
          !issue.number
            .toString()
            .charAt(0)
            .startsWith(numberParsedNumber.toString().charAt(0))
        ) {
          continue;
        }
      }
      ...
      issueResults.push({
        name: cutText(
          `(${parsedIssueState}) - ${issue.number} - ${issue.title}`,
          AutoCompleteLimits.MaximumLengthOfNameOfOption
        ),
        value: issue.number,
      });
    ...
    if (!issueResults.length) {
      ...
      const parsedIssueState =
        is?.state === "OPEN" ? "Open Issue" : "Closed Issue";

      issueResults.push({
        name: cutText(
          `(${parsedIssueState}) - ${is.number} - ${is.title}`,
          AutoCompleteLimits.MaximumLengthOfNameOfOption
        ),
        value: is.number,
      });
  ...
  if (!isNullishOrEmpty(pullRequests)) {
    ...
    if (!pullRequestResults.length) {
      ...
      pullRequestResults.push({
        name: cutText(
          `(${parsedPullRequestState}) - ${req.number} - ${req.title}`,
          AutoCompleteLimits.MaximumLengthOfNameOfOption
        ),
        value: req.number,
      });
...
function getDataForIssue(
  ...
  return {
    ...
    emoji:
      issue?.state === "OPEN"
        ? Emojis.GithubIssueOpen
        : Emojis.GithubIssueClosed,
    issueOrPr: "ISSUE",
    number: issue?.number,
...
function getDataForPullRequest(
