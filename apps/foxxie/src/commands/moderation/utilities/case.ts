...
import { FTFunction, PermissionLevels } from '#lib/types';
...
import { getModeration, resolveClientColor } from '#utils/functions';
...
export class UserCommand extends FoxxieSubcommand {
	...
	#formatTypeVariation(type: TypeVariation, t: FTFunction): string {
		switch (type) {
	...
	#listDetailsEntry(t: FTFunction, entry: ModerationManager.Entry, displayUser: boolean): EmbedField {
		...
		return {
			inline: false,
			name: `${inlineCode(entry.id.toString())} → ${getTitle(t as TFunction, entry)}`,
			value: lines.join('\n')
		};
	}
	...
	#parseListFooter(user: null | User, moderator: null | User, type: null | TypeVariation, pendingOnly: boolean, guild: Guild, t: FTFunction) {
		const parts: string[] = [];
		const { name } = guild;
