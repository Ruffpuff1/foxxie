...
import { APIAuditLogChangeKey$Remove, AuditLogEvent } from 'discord.js';

export class UnmuteLoggerTypeManager extends LoggerTypeManager {
	...
	protected override async filterAuditLogEntry(entry: LoggerTypeManager.AuditLogEntry) {
		...
		const change = entry.changes.find((change) => change.key === '$remove') as APIAuditLogChangeKey$Remove | undefined;
