...
export async function promptWithMessage(message: Message, options: AskYesNoOptions | string): Promise<[null | boolean, Message]> {
    if (typeof options === 'string') options = { content: options };

    const response = await send(message, options);
    const shouldPrompt = canReact(response.channel) ? await askYesOrNo(message, response, options) : await askConfirmationMessage(message, response, options);
