module.exports = {
    ...
    execute(message, args) { 
      if (!args[0]) { return message.channel.send('Well yknow, thats not correct. you have to do \`hk se (emote/image`)\` thanks.'); } 
      if (!args[1]) { return message.channel.send('Well yknow, thats not correct. you have to do \`hk se (emote/image`)\` thanks.'); }  
       const regex = /<(a?):(\w+):(\d+)>/
       const emotee = (`${args[0]}`).match(regex)
       const image = (`${args[0]}`).match(/\b(https?:\/\/\S+(png|jpe?g|gif)\S*)\b/i)
      if (!emotee && !image) { return message.channel.send('Well yknow, thats not correct. you have to do \`hk se (emote/image`)\` thanks.'); }
      if (image) {
