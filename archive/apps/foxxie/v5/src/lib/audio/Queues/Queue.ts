...
export class Queue {
  ...
  public async start(replaying = false): Promise<boolean> {
    ...
    container.client.emit(
      replaying ? Events.MusicSongReplayNotify : Events.MusicSongPlayNotify,
      this,
      nowPlaying
    );
    return true;
  ...
  public async getSystemPaused(): Promise<boolean> {
    return this.store.redis
      .fetch(this.redisKeys.systemPause)
      .then((d) => d === "1");
  }

  public async setSystemPaused(value: boolean): Promise<boolean> {
    await this.store.redis.insert(
  ...
  public async add(...tracks: readonly Song[]): Promise<number> {
    ...
    await this.store.redis.lpush(
      this.redisKeys.next,
      ...map(tracks.values(), serializeEntry)
    );
  ...
  public async songs(start = 0, end = -1) {
    ...
    const tracks = await this.store.redis.lrange(
      this.redisKeys.next,
      start,
      end
    );
  ...
  public async nowPlaying(): Promise<NP | null> {
    const [entry, position] = await Promise.all([
      this.getCurrentSong(),
      this.store.redis.get(this.redisKeys.position),
    ]);
  ...
  public async createPlaylist(): Promise<[Hash, string[]]> {
    ...
    const hash = `${Date.now().toString(36)}${
      container.client.options.shardCount
        ? container.client.options.shardCount.toString(36)
        : (1).toString(36)
    }`;
    // return the hash.
