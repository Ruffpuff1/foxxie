...
export class GuildModerationManager extends Collection<
  number,
  ModerationEntity
> {
  ...
  public async fetch(
    id: string | number[]
  ): Promise<Collection<number, ModerationEntity>>;
  public async fetch(id?: null): Promise<this>;
  public async fetch(
    id?: number | number[] | null | string
  ): Promise<
    ModerationEntity | null | Collection<number, ModerationEntity> | this
  > {
    if (!id) {
    ...
    if (Array.isArray(id)) {
      const entries = await Promise.all(
        id.map(async (caseId) =>
          this.db.fetchModerationEntry(caseId, this.guild?.id as string)
        )
      );
    ...
    return (
      super.get(id) ||
      this._cache(
        await this.db.fetchModerationEntry(
          id as number,
          (this.guild as Guild).id
        )
      )
    );
  }
  ...
  public insert(
    data: ModerationEntity | ModerationEntity[]
  ): Collection<number, ModerationEntity> | ModerationEntity | null {
    return this._cache(data, "insert");
  }

  private _cache(
    entries: ModerationEntity | ModerationEntity[] | undefined,
    type?: string
  ): Collection<number, ModerationEntity> | ModerationEntity | null {
    ...
    return Array.isArray(entries)
      ? new Collection(entries.map((entry) => [entry.caseId, entry]))
      : entries;
  }
