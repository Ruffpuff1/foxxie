...
export class SchemaKey<K extends keyof GuildEntity = keyof GuildEntity>
  implements ISchemaValue
{
  ...
  public display(settings: GuildEntity, t: TFunction): string {
    ...
    if (this.array) {
      ...
      return isNullish(values) || values.length === 0
        ...
        : `[ ${values
            .map((value) => serializer.stringify(value, context))
            .join(" | ")} ]`;
    }

    const value = settings[this.property];
    return isNullish(value)
      ? t(LanguageKeys.Commands.Admin.ConfNotSet)
      : serializer.stringify(value, context);
  }
  ...
  public get serializer(): Serializer<GuildEntity[K]> {
    ...
    if (typeof value === "undefined")
      throw new Error(`The serializer for '${this.type}' does not exist.`);
