...
export class GiveawayEntity extends BaseEntity {
  ...
  public async post() {
    ...
    const channel = await resolveToNull(
    ...
    if (!this.emoji) {
      ...
      this.emoji = rawEmoji.includes("%")
        ? rawEmoji
        : encodeURIComponent(rawEmoji.replace(/[<>]/g, ""));
    }
    ...
    if (!this.messageId) {
      ...
      const formattedEmoji = TwemojiRegex.test(decodedEmoji)
        ? decodedEmoji
        : `<${decodedEmoji}>`;

      const author = await this.fetchAuthor();
      const embed = formatInitialGiveawayEmbed(
        this.guild!,
        author!,
        formattedEmoji,
        this.#t,
    ...
    try {
      ...
      await api()
        .channels(this.channelId)
        .messages(this.messageId)
        .reactions(defaultEmoji, "@me")
        .put();
      this.emoji = defaultEmoji;
      await writeSettings(
        this.guild!,
        (settings) => (settings.giveawayEmoji = defaultEmoji)
      );
    }
  ...
  public async end() {
    ...
    if (winners === null || !winners.length)
      sent = await this.endWithNoWinner();
    ...
    if (this.#createdByBot) {
      ...
      description.push(
        this.#t("listeners/events:giveawayEmbedEnded", {
          time: time(
            Math.round(this.endsAt?.getTime() / 1000),
            TimestampStyles.RelativeTime
          ),
        }),
        `> [${this.#t("listeners/events:starboardJumpTo")}](${messageLink(
          this.guildId,
          this.channelId,
          sent.id
        )})`
  ...
  public async fetchWinners() {
    const participants = await this.filter([
  ...
  public async filter(users: string[]) {
    ...
    for (const user of users) {
      ...
      if (this.requiredRole && !member.roles.cache.has(this.requiredRole))
        continue;
  ...
  private async endWithWinner(winners: string[]) {
    const mapped = winners
      .map((id) => this.guild!.members.cache.get(id) ?? null)
      .filter((a) => Boolean(a));
    if (!mapped.length) return this.endWithNoWinner();

    const author = await this.fetchAuthor();

    const [shouldReply, shouldPing] = await acquireSettings(this.guild!, [
    ...
    if (shouldPing)
      data.content = mapped.map((member) => member!.toString()).join(", ");
  ...
  private async endWithNoWinner() {
    ...
    const shouldReply = await acquireSettings(
      this.guild!,
      "giveawayReplyToMessage"
    );
  ...
  public get link(): string {
    return `[${this.#t("listeners/events:starboardJumpTo")}](${messageLink(
      this.guildId,
      this.channelId,
      this.messageId!
    )})\n`;
