...
export class GuildEntity extends BaseEntity {
  ...
  @Column("varchar", {
    name: "channelsLogsFilterInvites",
    nullable: true,
    length: 19,
  })
  ...
  public getUserHighlight(
    user: UserResolvable
  ): Highlight<HighlightTypeEnum.Regex | HighlightTypeEnum.Word>[] {
  ...
  protected entityLoad() {
    this.wordFilterRegExp = this.words.length
      ? create(this.words.map((en) => en.word))
      : null;
  }
