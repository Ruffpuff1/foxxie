...
export class ModerationEntity extends BaseEntity {
  ...
  public constructor(data: Partial<ModerationEntity>) {
    ...
    if (data) {
      ...
      this.type = ModerationEntity.getTypeFromDuration(
        this.type as number,
        this.duration
      );
    }
  ...
  public clone(): ModerationEntity {
    return new ModerationEntity(this).setup(
      this.#manager as GuildModerationManager
    );
  }
  ...
  public edit(
    data: ModerationManagerDescriptionData | Record<string, unknown> = {}
  ): ModerationEntity {
    const dataWithType = {
      ...data,
  ...
  public async fetchLogChannel(): Promise<GuildChannel | null> {
    ...
    const channel = this.logChannelId
      ? ((await resolveToNull(
          this.guild!.channels.fetch(this.logChannelId)
        )) as GuildChannel)
      : null;
  ...
  public async fetchModerator(): Promise<User | null> {
    ...
    const moderator = this.moderatorId
      ? await resolveToNull(this.client.users.fetch(this.moderatorId))
      : null;
  ...
  public async formats(): Promise<{ title: string; t: TFunction }> {
    const t = await acquireSettings(this.guildId!, (settings) =>
  ...
  private async fetchDescriptionData(mod: User): Promise<(string | null)[]> {
    const [prefix, t] = await acquireSettings(this.guildId!, (settings) => [
      settings[GuildSettings.Prefix],
      settings.getLanguage(),
    ]);

    const [user, channel, refrence] = await Promise.all([
      this.fetchUser(),
      this.fetchChannel(),
      this.fetchRefrenceCase(),
    ]);
    ...
    return [
      ...
      refrence
        ? t(LanguageKeys.Guilds.Logs.ArgsRefrence, {
            ...
            url: messageLink(
              this.guildId!,
              refrence.logChannelId!,
              refrence.logMessageId!
            ),
          })
        : null,
    ].filter((a) => a !== null);
  ...
  private static getTypeFromDuration(
    type: number,
    duration: number | null
  ): number {
    if (duration === null) return type;
    if (duration < Time.Minute)
      return type | TypeMetadata.Temporary | TypeMetadata.Fast;
  ...
  public get guild(): Guild | null {
    return (
      this.#manager?.guild ||
      container.client.guilds.cache.get(this.guildId!) ||
      null
    );
  }
  ...
  private get metadata(): MetaData {
    ...
    if (typeof data === "undefined")
      throw new Error(
        `Inexistent metadata for '0b${this.type
          ?.toString(2)
          .padStart(8, "0")}'.`
      );
    return data;
  ...
  private get shouldSend(): boolean {
    ...
    for (const entry of this.#manager!.values()) {
      if (this.userId !== entry.userId || before > entry.createdTimestamp)
        continue;

      if (type === entry.type) return false;
      if (
        isMute &&
        (entry.type === TypeCodes.TemporaryMute ||
          entry.type === TypeCodes.Mute)
      )
        return false;
