...
export class GuildSettingsManager<T extends GuildEntity> extends Collection<
  string,
  T
> {
  ...
  public acquire<K1 extends keyof T>(
    key: string,
    paths: readonly [K1]
  ): Promise<[T[K1]]>;
  public acquire<K1 extends keyof T, K2 extends keyof T>(
    key: string,
  ...
  public acquire<K extends keyof T>(
    key: string,
    paths: readonly K[]
  ): Promise<T[K][]>;
  ...
  public acquire<R>(
    key: string,
    cb: SettingsCollectionCallback<T, R>
  ): Promise<R>;
  ...
  public write<K1 extends keyof T>(
    key: string,
    pairs: readonly [[K1, T[K1]]]
  ): Promise<void>;
  public write<K1 extends keyof T, K2 extends keyof T>(
    key: string,
    pairs: readonly [[K1, T[K1]], [K2, T[K2]]]
  ): Promise<void>;
  ...
  public write<K extends keyof T>(
    key: string,
    pairs: readonly [K, T[K]][]
  ): Promise<void>;
  public write<R>(
    key: string,
    cb: SettingsCollectionCallback<T, R>
  ...
  public async write<R>(
    ...
    const found =
      this.get(key) || (await this.unlockOnThrow(this.processFetch(key), lock));
