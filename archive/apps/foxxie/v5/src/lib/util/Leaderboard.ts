...
export class Leaderboard {
    ...
    public async fetch(type: LeaderboardType, guild?: string): Promise<Collection<string, LeaderboardUser>> {
        if (guild && type === LeaderboardType.Local) {
            if (this.promises.guilds.has(guild)) await this.promises.guilds.get(guild);
    ...
    private async syncMembers(guild: string): Promise<void> {
        ...
        const promise = new Promise<void>(resolve => this.createMemberSync(guild).then(resolve));
    ...
    private async createMemberSync(guild: string) {
        ...
        for (const entry of data.filter(entry => entry.member !== undefined)) {
            store.set(entry.id, { points: entry.points, position: ++i, level: entry.level });
        }
