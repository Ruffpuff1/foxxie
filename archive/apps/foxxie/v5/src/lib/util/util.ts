...
export async function fetchReactionUsers(
  ...
  do {
    ...
    for (const user of rawUsers.filter(
...
export async function sendLoadingMessage(
  ...
  key:
    ...
    | CustomFunctionGet<any, string, string[]> = LanguageKeys.System
    .MessageLoading,
  args = {},
  ephemeral = false
): Promise<Message> {
  ...
  const content = Array.isArray(translated)
    ? randomArray(translated)
    : translated;
  ...
  const f = (x: number) =>
    ...
    Math.min(
      Math.max(
        0,
        2 *
          (10 * ((x - 5) / 10 - Math.floor(1 / 2 + (x - 5) / 10)) ** 2 +
            10 * Math.floor(x / 10) +
            x -
            2.5)
      ),
...
export async function canDmUser(user: User): Promise<boolean> {
  ...
  const data = await fetch(
    `${container.client.options.http!.api}/v${
      container.client.options.http!.version
    }`,
    "POST"
  ) //
    .path("channels")
  ...
  if (
    data.code! &&
    data.code === RESTJSONErrorCodes.CannotSendMessagesToThisUser
  )
    return false;
  return true;
}
...
export async function fetchTasks<T extends Schedules = Schedules.PostAnalytics>(
  type?: T,
  filter: (j: Job) => boolean = () => true
): Promise<MappedJob<T>[]> {
  const cache = (await container.tasks.list({ start: 0 })) as Job[];

  const filtered = cache.filter((a) => Boolean(a)).filter(filter);
  const list = type
    ? filtered.filter((job) => job.data.task === type)
    : filtered;
...
export function sortTasksByTime<T extends Schedules = Schedules.PostAnalytics>(
  tasks: MappedJob<T>[],
  reverse = false
) {
...
export function getServerDetails(): ServerDetails {
  ...
  return {
    ...
    memoryUsed,
    memoryPercent: (
      (parseInt(memoryUsed, 10) / parseInt(totalmemory, 10)) *
      100
    ).toFixed(1),
    process: hostname(),
