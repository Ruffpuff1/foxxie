...
export class GuildMemberFetchQueue {
   ...
   */
  public add(shardId: number, guildId: string): void {
   ...
   */
  public remove(shardId: number, guildId: string): void {
    const entry = this.shards.get(shardId);
    if (!entry) return;
   ...
   */
  private fetchShard(entry: GuildMemberFetchQueueShardEntry): void {
    // There must be less than 100 entries being fetched from the same shard, and at least one entry pending:
    while (
      entry.fetching < kMaximumQueriesPerMinute &&
